...
export abstract class BaseAdapter<T, E extends BaseEntity>
  implements BaseRepository<T, E>
{
  ...
  async saveMany(entity: T[] & E[]): Promise<void> {
    await this.adapterRepository.save(entity);
  }

  async findByOptions(findOneOptions: FindOneOptions<E>): Promise<T | null> {
    const templateExist = await this.adapterRepository
  ...
  async findAll(
    findOptions?: FindManyOptions<E> | undefined,
  ): Promise<[T[], number]> {
    const [entities, count] = await this.adapterRepository
  ...
  async deleteByOptions(findOptions: FindOptionsWhere<E>): Promise<boolean> {
    const result = await this.adapterRepository
      .delete(findOptions)
      .catch((err) => {
        this.logger.error(err);
        return false;
